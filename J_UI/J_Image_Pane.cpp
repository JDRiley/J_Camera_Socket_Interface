...
J_Image_Pane::J_Image_Pane(const J_Rectangle& ik_rec
	...
	M_format = i_format;
	s_ui_model->notify_image_input_format(get_ID(), i_format);
	s_ui_model->notify_new_image_buffer(get_ID(), nullptr);
}

void J_Image_Pane::set_buffer(const j_ubyte* i_buffer){
	if(i_buffer){
		s_ui_model->notify_new_image_buffer(get_ID(), i_buffer);
	}else{
		s_ui_model->notify_image_clear(get_ID());
	}
}
...
void J_Image_Pane::set_image_width(int i_width){
	assert(i_width >= 0);
	M_width = i_width;
	s_ui_model->notify_image_width(get_ID(), i_width);
}

void J_Image_Pane::set_image_height(int i_height){
	assert(i_height >= 0);
	M_height = i_height;
	s_ui_model->notify_image_height(get_ID(), i_height);
}
...
void J_Image_Pane::broadcast_current_state()const{
	...
	s_ui_model->notify_image_width(get_ID(), M_width);
	s_ui_model->notify_image_height(get_ID(), M_height);
	s_ui_model->notify_image_input_format(get_ID(), M_format);
}
...
void J_Image_Pane::set_middle_line_color(J_UI_Color i_color){
	...
	s_ui_model->notify_middle_line_color(get_ID(), M_middle_line_color);
}
