...
J_Font_Manager::J_Font_Manager(){
	...
	M_context  = s_contexts->create_j_context(10,10,"This J_Font_Manager Window is Invisible"
		, nullptr, nullptr);
}
...
Bitmap_Metrics::Bitmap_Metrics(int i_width, int i_height, int i_left
				...
				:width(i_width), height(i_height), left(i_left), underreach(i_underreach)
...
J_Font_Face__::~J_Font_Face__(){
	glDeleteTextures(static_cast<GLsizei>(M_texture_ids.size()), M_texture_ids.data());
}
...
J_Font_Face J_Font_Manager::get_font_face(J_Context_Shared_t i_context_to_add_to
	...
	ex_array<ex_array<j_ubyte>> glyph_datas(source_font_face->M_texture_ids.size());

	for(int i=0; i < source_font_face->M_texture_ids.size(); i++){
		glyph_datas[i].resize(source_font_face->M_bitmap_metrics[i].height
			*(source_font_face->M_bitmap_metrics[i].width));

		glBindTexture(GL_TEXTURE_2D, source_font_face->M_texture_ids[i]);
		glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_UNSIGNED_BYTE, glyph_datas[i].data());
	}

	J_Font_Face font_face_to_return = new J_Font_Face__(*source_font_face);
	fill(font_face_to_return->M_texture_ids.begin(), font_face_to_return->M_texture_ids.end(), 0);

	int num_glyphs = safe_int_cast(font_face_to_return->M_bitmap_metrics.size());
	s_contexts->make_context_active(i_context_to_add_to);
	glGenTextures(num_glyphs, font_face_to_return->M_texture_ids.data());
	
	for(int i=0; i < num_glyphs; i++){
		int width = font_face_to_return->M_bitmap_metrics[i].width;
		int height = font_face_to_return->M_bitmap_metrics[i].height;
		
		glBindTexture(GL_TEXTURE_2D, font_face_to_return->M_texture_ids[i]);
		glTexStorage2D(GL_TEXTURE_2D, 1, GL_R8, width, height);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RED
			, GL_UNSIGNED_BYTE, glyph_datas[i].data());
